...
function stopAllSims(){
  stopAudio();
  clearInterval(window.dyslexiaInterval); clearInterval(window.adhdInterval); clearInterval(window.autismInterval); clearInterval(window.visionInterval);
  $('#dyslexiaTimer').textContent='20';
  $('#dyslexiaText').innerHTML='';
  $('#distractors').innerHTML=''; $('.adhd-screen').classList.remove('active-noise');
  $('.autism-screen').classList.remove('flash','overload');
  $('#visionTimer').textContent='10'; $('#visionInput').value=''; $('#visionResult').textContent=''; $('#visionCard').classList.add('blurred');
}

const dyslexiaBase = 'Özel eğitim bireyin farklı öğrenme biçimini anlamak ve ona uygun destek sunmak için vardır.';
const dyslexiaWords = dyslexiaBase.split(' ');
const lookAlikes = { a:'e', e:'a', ı:'i', i:'ı', o:'ö', ö:'o', u:'ü', ü:'u', b:'d', d:'b', p:'q', m:'n', n:'m', s:'ş', c:'ç' };
function distortWord(word, index){
  const chars = word.split('');
  if(word.length < 4 || index % 4 === 0) return `<span>${word}</span>`;
  for(let i=1;i<chars.length-1;i++){
    const lower = chars[i].toLowerCase();
    if(Math.random()<0.26 && lookAlikes[lower]) chars[i] = chars[i]===chars[i].toUpperCase()?lookAlikes[lower].toUpperCase():lookAlikes[lower];
    if(Math.random()<0.22 && i<chars.length-2){ [chars[i],chars[i+1]]=[chars[i+1],chars[i]]; i++; }
  }
  const cls = Math.random()<0.42 ? 'jump-letter' : '';
  return `<span class="${cls}">${chars.join('')}</span>`;
}
function renderDyslexia(){ $('#dyslexiaText').innerHTML = dyslexiaWords.map(distortWord).join(' '); }
$('#startDyslexia').addEventListener('click', () => {
  let t = 20; $('#dyslexiaTimer').textContent=t; clearInterval(window.dyslexiaInterval); renderDyslexia();
  window.dyslexiaInterval = setInterval(()=>{
    t--; $('#dyslexiaTimer').textContent=t; renderDyslexia();
    if(t<=0){ clearInterval(window.dyslexiaInterval); $('#dyslexiaText').innerHTML='<span>Bazı bireyler için okuma süreci sadece birkaç saniyelik değil, her gün yaşanan bir çabadır.</span>'; }
  }, 430);
});

$('#startAdhd').addEventListener('click', () => {
  const wrap = $('#distractors'); wrap.innerHTML=''; $('.adhd-screen').classList.add('active-noise'); noise(.8,.035);
  const words = ['Bildirim!', 'Ses!', 'Hareket!', 'Biri konuşuyor', 'Yeni görev!', 'Dikkat dağıldı', 'Bak buraya!', 'Acele!', 'Telefon!', 'Kapı çaldı'];
  clearInterval(window.adhdInterval);
  window.adhdInterval = setInterval(()=>{
    tone(160 + Math.random()*980, .10, Math.random()>.5?'square':'triangle', .018);
    for(let i=0;i<2;i++){
      const s = document.createElement('span'); s.textContent = words[Math.floor(Math.random()*words.length)];
      s.style.left = Math.random()*82 + 5 + '%'; s.style.top = Math.random()*66 + 18 + '%';
      s.style.transform = `rotate(${Math.random()*26-13}deg)`;
      wrap.appendChild(s); setTimeout(()=>s.remove(), 2300);
    }
  }, 360);
  setTimeout(()=>{clearInterval(window.adhdInterval); $('.adhd-screen').classList.remove('active-noise');}, 10000);
});

$('#startAutism').addEventListener('click', () => {
  $('.autism-screen').addClass?.('flash');
  $('.autism-screen').classList.add('flash','overload');
  clearInterval(window.autismInterval);
  window.autismInterval = setInterval(()=>{
    tone(280+Math.random()*1500,.07,Math.random()>.5?'sawtooth':'square',.018);
    if(Math.random()>.55) noise(.18,.018);
  }, 130);
  setTimeout(()=>{clearInterval(window.autismInterval); $('.autism-screen').classList.remove('flash','overload'); stopAudio();}, 7000);
});

const visionSentence = 'Erişilebilir tasarım herkes için önemlidir.';
$('#visionCard').textContent = visionSentence;
function normalizeTR(s){ return s.toLocaleLowerCase('tr-TR').replace(/[.,;:!?]/g,'').replace(/\s+/g,' ').trim(); }
let visionStarted = false;
$('#checkVision').addEventListener('click', () => {
  if(!visionStarted){
    visionStarted = true; let t=10; $('#visionTimer').textContent=t; $('#visionResult').textContent='Süre başladı. Cümleyi yazmaya çalış.';
    clearInterval(window.visionInterval);
    window.visionInterval=setInterval(()=>{ t--; $('#visionTimer').textContent=t; if(t<=0){clearInterval(window.visionInterval); $('#visionResult').textContent='Süre bitti. Şimdi cevabını kontrol edebilirsin.';} },1000);
    return;
  }
  const ok = normalizeTR($('#visionInput').value) === normalizeTR(visionSentence);
  if(ok){
    $('#visionResult').textContent = 'Tebrikler, doğru yazdın. Ama sen bunu sadece 10 saniye deneyimledin; görme yetersizliği olan bireyler günlük yaşamda çok daha uzun süre bu tür engellerle karşılaşabilir.';
    $('#visionCard').classList.remove('blurred');
  }else{
    $('#visionResult').textContent = 'Cevap tam doğru değil. Bu zorluk, erişilebilir tasarımın neden önemli olduğunu gösterir.';
  }
});
